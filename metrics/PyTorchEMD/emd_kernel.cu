...
#include <cmath>
#include <vector>
#include <algorithm> // For min/max, which might be cleaner than using a macro
...
// Helper to get the minimum of two values, as min is not universally available in CUDA headers
template<typename T>
__device__ T dmin(T a, T b) {
    return a < b ? a : b;
}
...
__global__ void approxmatch(int b,int n,int m,const scalar_t * __restrict__ xyz1,const scalar_t * __restrict__ xyz2,scalar_t * __restrict__ match,scalar_t * temp){
    ...
    if (n>=m){
        multiL=1;
        multiR=(scalar_t)n/m; // Cast to scalar_t for floating point division
    }else{
        multiL=(scalar_t)m/n; // Cast to scalar_t for floating point division
        multiR=1;
    }
    const int Block=1024;
    __shared__ scalar_t buf[Block*4];
    for (int i=blockIdx.x;i<b;i+=gridDim.x){
        // Initialize match, remainL, remainR
        ...
        for (int j=7;j>=-2;j--){
            ...
            // Second loop: calculate ratioR and update remainR
            for (int l0=0;l0<m;l0+=blockDim.x){
                ...
                for (int k0=0;k0<n;k0+=Block){
                    ...
                    for (int k=0;k<kend;k++){
                        ...
                        scalar_t w=expf(level*((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)+(z2-z1)*(z2-z1)))*buf[k*4+3];
                        sumr+=w;
                    }
                    __syncthreads();
                }
            ...
            // Third loop: calculate match and update remainL
            for (int k0=0;k0<n;k0+=blockDim.x){
                int k=k0+threadIdx.x;
                scalar_t x1=0,y1=0,z1=0;
                if (k<n){
                ...
                for (int l0=0;l0<m;l0+=Block){
                    ...
                    if (k<n){
                        for (int l=0;l<lend;l++){
                            ...
                            scalar_t y2=buf[l*4+1];
                            scalar_t z2=buf[l*4+2];
                            scalar_t w=expf(level*((x2-x1)*(x2-x1)+(y2-y1)*(y2-y1)+(z2-z1)*(z2-z1)))*rl*buf[l*4+3];
                            // Note: match is (B, M, N) here, access is match[i*M*N + k*N + j] where k is xyz2 index (M) and j is xyz1 index (N)
                            // The user's original code used match[i*n*m+(l0+l)*n+k] which suggests shape (B, M, N).
                            match[i*m*n+(l0+l)*n+k]+=w;
                            suml+=w;
                        }
                    }
                    __syncthreads();
...
at::Tensor ApproxMatchForward(
    ...
    TORCH_CHECK(xyz2.size(0) == b, "xyz2 batch size must equal to xyz1 batch size");
    TORCH_CHECK(xyz1.size(2) == 3, "xyz1 last dim must be 3");
    TORCH_CHECK(xyz2.size(2) == 3, "xyz2 last dim must be 3");
    CHECK_INPUT(xyz1);
    CHECK_INPUT(xyz2);

    auto match = at::zeros({b, m, n}, xyz1.options()); // Use .options() instead of .type()
    auto temp = at::zeros({b, (n+m)*2}, xyz1.options());

    AT_DISPATCH_FLOATING_TYPES(xyz1.scalar_type(), "ApproxMatchForward", ([&] {
        approxmatch<scalar_t><<<32,512>>>(b, n, m, xyz1.data_ptr<scalar_t>(), xyz2.data_ptr<scalar_t>(), match.data_ptr<scalar_t>(), temp.data_ptr<scalar_t>());
    }));
...
at::Tensor MatchCostForward(
    ...
    CHECK_INPUT(xyz1);
    CHECK_INPUT(xyz2);
    CHECK_INPUT(match); // Added check for match tensor

    auto cost = at::zeros({b}, xyz1.options());

    AT_DISPATCH_FLOATING_TYPES(xyz1.scalar_type(), "MatchCostForward", ([&] {
        matchcost<scalar_t><<<32,512>>>(b, n, m, xyz1.data_ptr<scalar_t>(), xyz2.data_ptr<scalar_t>(), match.data_ptr<scalar_t>(), cost.data_ptr<scalar_t>());
    }));
    AT_CUDA_CHECK(cudaGetLastError());
    ...
    for (int i=blockIdx.x;i<b;i+=gridDim.x){
        ...
        for (int k=kbeg;k<kend;k++){
            ...
            for (int j=threadIdx.x;j<n;j+=blockDim.x){
                ...
                scalar_t d=match[i*m*n+k*n+j]*2; // match is (B, M, N)
                subsumx+=x1*d;
                subsumy+=y1*d;
                subsumz+=z1*d;
            }
    ...
    for (int i=blockIdx.x;i<b;i+=gridDim.x){
        for (int l=threadIdx.x;l<n;l+=blockDim.x){
            ...
            for (int k=0;k<m;k++){
                ...
                scalar_t d=match[i*m*n+k*n+l]*2; // match is (B, M, N)
                dx+=(x1-x2)*d;
                dy+=(y1-y2)*d;
                dz+=(z1-z2)*d;
            }
...
std::vector<at::Tensor> MatchCostBackward(
    ...
    AT_DISPATCH_FLOATING_TYPES(xyz1.scalar_type(), "MatchCostBackward", ([&] {
        matchcostgrad1<scalar_t><<<32,512>>>(b, n, m, grad_cost.data_ptr<scalar_t>(), xyz1.data_ptr<scalar_t>(), xyz2.data_ptr<scalar_t>(), match.data_ptr<scalar_t>(), grad1.data_ptr<scalar_t>());
        matchcostgrad2<scalar_t><<<dim3(32,32),256>>>(b, n, m, grad_cost.data_ptr<scalar_t>(), xyz1.data_ptr<scalar_t>(), xyz2.data_ptr<scalar_t>(), match.data_ptr<scalar_t>(), grad2.data_ptr<scalar_t>());
    }));
    AT_CUDA_CHECK(cudaGetLastError());
